...
const ProductSingle = ({product,progress,cls,offer_style,price_space}:IProps) => {
  const {image_url,price,sale_price,name,updated_at,quantity,sold,variants,category_name,subcategory_name,offerDate,reviews} = product || {};
    ...
    >
      <div className="tpproduct__thumb p-relative text-center">
        <Link href={`/shop-details/${product.id}`}>
          <Image
            src={`${process.env.NEXT_PUBLIC_IMAGE_URL}${image_url}`}
            alt="product-img"
            width={217}
            height={217}
            style={imgStyle}
      ...
      <div className="tpproduct__content">
        ...
        >
          <Link href={`/shop-details/${product.id}`}>{subcategory_name}</Link>
        </span>
        <h4 className="tpproduct__title">
          <Link href={`/shop-details/${product.id}`}>{name}</Link>
        </h4>
        <div className="tpproduct__rating mb-5">
          ...
          >
            <span>
              ₹ {sale_price ? sale_price.toFixed(2) : parseFloat(variants[0].discounted_price || "0").toFixed(2)}
            </span>
            {<del>${parseFloat(variants[0].price || "0").toFixed(2)}</del>}
          </div>
        )}
      ...
      <div className="tpproduct__hover-text">
        ...
        <div className="tpproduct__descrip">
          <ul>
            <li>Category: {product.subcategory_name}</li>
            <li>LIFE: 3days</li>
          </ul>
        </div>
      </div>
